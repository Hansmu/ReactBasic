...
const VideoList = (props) => {
    ...
    const videoItems = props.videos.map((video) => {
       return ( //Passing the callback function down to the VideoListItem
           <VideoListItem
               onVideoSelect={props.onVideoSelect}
               key={video.etag}
               video={video} />
       );//Has to have IDs for specific elements, so it wouldn't re-render all of the elements when one changes.
    });
